...
CREATE POLICY IF NOT EXISTS "Users can manage own focus sessions" ON focus_sessions 
...
CREATE POLICY IF NOT EXISTS "Users can manage own weekly reviews" ON weekly_reviews 
  FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY IF NOT EXISTS "Users can view own habit streaks" ON habit_streaks FOR SELECT USING (
...
CREATE POLICY IF NOT EXISTS "Users can manage own goal milestones" ON goal_milestones FOR ALL USING (
